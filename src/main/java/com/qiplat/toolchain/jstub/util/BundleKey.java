package com.qiplat.toolchain.jstub.util;

/**
 * bundle key constants
 * @author Scave
 */
public interface BundleKey {
    /**
     * help
     */
    String help_head = "help_head";
    String usage = "help_usage";
    String help_options_head = "help_options_head";
    String help_out = "help_out";
    String help_level = "help_level";
    String help_level0 = "help_level0";
    String help_level1 = "help_level1";
    String help_level2 = "help_level2";
    String help_lower_bytecode = "help_lower_bytecode";
    String help_verbose = "help_verbose";
    String help_command = "help_command";

    /**
     * errors
     */
    String no_error = "no_error";
    String file_error = "file_error";
    String out_error = "out_error";
    String out_must_error = "out_must_error";
    String level_error = "level_error";
    String level_param_error = "level_param_error";

    /**
     * messages
     */
    String prepare_process = "prepare_process";
    String process_tip = "process_tip";
}
